...
export function ArcanaSiteShell({ children }: { children: ReactNode }) {
  return (
    <main className="min-h-dvh overflow-y-auto bg-void text-parchment">
      <header className="sticky top-0 z-40 border-b border-line/80 bg-void/95 px-4 py-3 backdrop-blur sm:px-6">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          <Link to="/" className="font-display text-sm tracking-[0.24em] text-gold">
            ARCANA SQUAD
          </Link>
          <nav className="flex max-w-[72vw] gap-1 overflow-x-auto pb-1 text-xs text-muted sm:gap-2" aria-label="Arcana Squad">
            {nav.map((item) => (
              <Link
                key={item.to}
                to={item.to}
                className="whitespace-nowrap rounded-full border border-line px-3 py-2 transition-colors hover:border-gold-dim hover:text-parchment"
